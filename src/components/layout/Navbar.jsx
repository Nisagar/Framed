...
export default function Navbar() {
  ...
  // ── Nav links — category links use ?category= param ──
  const navLinks = [
  ...
  function isActive(href) {
    ...
    if (href !== '/products' && currentFull === href) return true
    return false
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/cart"
              className="relative p-2 text-brand-dark/70 hover:text-brand-primary transition-colors"
              aria-label="Cart"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
              ...
              {totalItems > 0 && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-brand-primary text-brand-bg
                ...
                <AnimatePresence>
                  {userMenuOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -8 }}
                      className="absolute right-0 top-full mt-2 w-48 bg-brand-bg border
                                 border-brand-border shadow-card-hover z-50"
                    >
                      <div className="py-1">
                        <Link to="/dashboard"
                          className="block px-4 py-2.5 text-sm font-body text-brand-dark hover:bg-brand-cream">
                          My Account
                        </Link>
                        <Link to="/dashboard"
                          className="block px-4 py-2.5 text-sm font-body text-brand-dark hover:bg-brand-cream">
                        ...
                        <Link to="/custom-upload"
                          className="block px-4 py-2.5 text-sm font-body text-brand-dark hover:bg-brand-cream">
                          Custom Upload
                        </Link>
                        {isAdmin && (
                          <Link to="/admin"
                            className="block px-4 py-2.5 text-sm font-body text-brand-primary
                                       font-medium hover:bg-brand-cream border-t border-brand-border mt-1">
                            Admin Panel
                          </Link>
                        )}
                        <button
                          onClick={handleSignOut}
                          className="block w-full text-left px-4 py-2.5 text-sm font-body
                                     text-red-600 hover:bg-red-50 border-t border-brand-border"
                        >
                          Sign Out
                        </button>
                      </div>
            ...
            <nav className="px-4 py-4 flex flex-col gap-1">
              {navLinks.map(link => (
                <Link
                  key={link.href}
                  to={link.href}
                  className={`font-body text-sm py-3 border-b border-brand-border/50 last:border-0
                    transition-colors ${
                    isActive(link.href)
                      ? 'text-brand-primary font-medium'
                      : 'text-brand-dark'
